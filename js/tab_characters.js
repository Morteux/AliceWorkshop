...
function getMenuCharacterHTML(character_data) {
    ...
    return `
    <div id="character_check_` + character_data.name + `" class="character_container" onclick="printCharacterInfoHTML('` + character_data.name + `')">
...
function printCharacterInfoHTML(character_name) {
    let character_data = characters[character_name];
    console.log(character_name);
    document.getElementById("menu_characters").style.display = "none";
    
    document.getElementById("menu_characters_info").innerHTML = `
    <img class="" src="https://api.ambr.top/assets/UI/` + character_data.images.nameicon + `.png">
    <img class="" src="https://api.ambr.top/assets/UI/` + character_data.images.namegachasplash + `.png">
    `;
}
